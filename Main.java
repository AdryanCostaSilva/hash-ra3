...
class DoubleHash{
	...
	public void insert(Random random, Registro list[], int size) {
		int key = random.nextInt(100000000, 1000000000); // new key
		int hash1 = key % size;
		...
		while (list[index] != null) {  // while spot is occupied
			collisionCount++;          // count each collision
			i++;
	...
	public boolean search(int key, Registro[] list, int size) {
		...
		while (list[index] != null) {
			...
			if (i >= size) break; // percorreu tudo
		}
...
class MultiplicationHash{
	...
	public void printTop3LongestLists() {
		int[] lengths = new int[size]; // store lengths of each list
		...
		for (int n = 0; n < 3; n++) {
			...
			if (maxIndex != -1 && maxLen > 0) {
				...
				lengths[maxIndex] = 0; // so it won't be picked again
			}
		}
	}


	// etodo getter para contar colisoes
	public long getCollisionCount() {
		return collisionCount;
	}
	...
	public void calculateGaps() {
		...
		// Check final gap if table ends with empty slots
		if (currentGap > 0) {
			gapCount++;
			totalGap += currentGap;
			if (currentGap > max) max = currentGap;
